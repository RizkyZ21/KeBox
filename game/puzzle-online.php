...
<title>Online Lobby Puzzle — KeBox</title>
...
<div class="container-sm" style="padding-top:3rem;padding-bottom:3rem">
    <div class="page-header">
        <h1>👥 Mode Online</h1>
        <p>Sliding Puzzle · Level <?= $levelNum ?></p>
    </div>
    ...
    <div class="card mb-3">
        ...
        <p style="color:var(--text-dim);font-size:.9rem;text-align:center;margin-bottom:1.2rem">Siapa yang lebih cepat selesaikan puzzle?</p>
...
async function createRoom() {
    ...
    await fetch('api-session.php', {
        ...
        body: JSON.stringify({ action:'create', code, level: 'puzzle-' + LEVEL, username: USERNAME })
    });
    ...
    pollInterval = setInterval(async () => {
        ...
        if (data.player2) {
            ...
            window.location.href = `puzzle-play.php?level=${LEVEL}&mode=2p&room=${code}&role=host`;
        }
    }, 2000);
}
...
async function joinRoom() {
    ...
    const data = await res.json();
    if (data.success) {
        window.location.href = `puzzle-play.php?level=${LEVEL}&mode=2p&room=${code}&role=guest`;
    } else {
        alert(data.error || 'Room tidak ditemukan.');
    }
}
