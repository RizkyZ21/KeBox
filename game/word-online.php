...
<title>&#x1F4AC; Word Game Online &#x2014; KeBox</title>
...
<div class="container-sm" style="padding-top:2.5rem;padding-bottom:3rem">
    <div class="page-header" style="margin-bottom:2rem">
        ...
        <p style="color:var(--text-dim)"><span class="badge badge-<?= $level ?>"><?= strtoupper($level) ?></span> &nbsp;&middot;&nbsp; 1 vs 1</p>
    </div>
...
async function createRoom() {
    try {
        ...
        const res  = await fetch('api-session.php', {
            ...
            body: JSON.stringify({ action:'create', code, level: 'word-' + GAME_LEVEL })
        });
...
function showLobby(role) {
    ...
    if (role === 'host') {
        ...
        fetch('api-session.php?action=get_state&code=' + currentCode)
            .then(r => r.json())
            .then(s => {
                if (s && s.player1) setSlot('slot1', s.player1, false);
            })
            .catch(() => {});
    }
    startPoll(role);
...
function setSlot(slotId, name, isMe) {
    ...
    // Simpan ID avatar/name agar fillSlot1 tetap bisa update setelah innerHTML diganti
...
function startPoll(role) {
    pollIv = setInterval(async () => {
        try {
            ...
            // Room didelete host → {cancelled:true} tanpa field lain
            if (!s || s.cancelled) {
                clearInterval(pollIv);
                if (role === 'guest') {
                    alert('Room telah dibubarkan oleh host.');
                    window.location.href = 'word-online.php?level=' + GAME_LEVEL;
                }
                return;
            }
            ...
            if (role === 'host') {
                ...
                // Guest pergi → reset slot2
                if (!s.player2 && slot2 && slot2.classList.contains('filled')) {
                    resetSlot2();
                }
            }

            // ── GUEST: tampilkan nama host kalau belum muncul ────
...
async function cancelLobby() {
    ...
    heartbeatIv = null;
    // Beritahu server agar room dibubarkan / guest keluar
    if (currentCode) {
        try {
